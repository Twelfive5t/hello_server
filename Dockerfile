...
# ── 1. 换源（必须在首次 apt update 之前完成，否则仍走原源）─────────────
RUN sed -i \
...
# ── 3a. 更新软件包索引（独立层：index 不变时后续 install 层可直接命中缓存）
RUN apt update
...
# ── 4. 安装 Conan 2（独立成层，包版本稳定时可复用缓存）──────────────────
RUN pip3 install --no-cache-dir conan

# ── 5. 安装 Clang 18（LLVM 官方脚本）──
RUN wget -qO /tmp/llvm.sh https://apt.llvm.org/llvm.sh \
    && chmod +x /tmp/llvm.sh \
    && /tmp/llvm.sh 18 all \
    && rm /tmp/llvm.sh

# ── 6. 将 Clang 18 注册为系统默认工具链 ────────────────────────────────────
